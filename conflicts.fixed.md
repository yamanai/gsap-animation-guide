...
:::tip 核心概念
动画冲突发生�?*多个动画同时尝试控制相同元素的相同属�?*时。GSAP提供了多种方式处理这些冲突，以确保动画按照预期运行�?
:::
...
<div class="concept-visual">
  <div class="timeline-view">
    ...
    <div class="visual-caption">
      没有冲突处理时，第二个动画会完全覆盖第一个动画的效果，导致不自然的运动路�?
    </div>
  </div>
</div>
...
<div class="options-comparison">
  <div class="option-card">
    ...
    <div class="option-desc">默认�?/div>
    <div class="option-explain">不覆盖任何现有动画，动画将并行运�?/div>
    <div class="option-code">
      ```js
      gsap.to(".element", { 
        x: 100, 
        duration: 1,
        // overwrite: false // 默认值，可省�?
      });
      ```
    </div>
    <div class="option-note">当多个动画控制相同属性时可能导致不可预测的结�?/div>
  </div>
  ...
  <div class="option-card">
    ...
    <div class="option-note">慎用，会停止元素上的所有动画，包括不冲突的属�?/div>
  </div>
  <div class="option-card">
    <div class="option-title">对象形式</div>
    <div class="option-desc">精确控制</div>
    <div class="option-explain">允许指定哪些特定属性应当覆盖现有动�?/div>
    <div class="option-code">
      ```js
      gsap.to(".element", { 
        x: 100, 
    ...
    <div class="option-note">最灵活的方式，适合复杂场景的精确控�?/div>
  </div>
</div>
...
// 对同一元素创建序列动画
gsap.to(".box", { x: 200, duration: 2 });
gsap.to(".box", { y: 100, duration: 1, delay: 1 }); // 不会冲突，控制不同属�?
gsap.to(".box", { x: 0, duration: 1, delay: 2 });   // 会与第一个动画冲�?/code></pre>
  </div>
  <div class="solution">
    <h4>�?解决方案</h4>
...
elem.addEventListener("click", () => {
  gsap.to(elem, { 
    ...
    duration: 0.5,
    overwrite: {
      scale: true, // 只覆盖scale属�?
      rotation: true // 只覆盖rotation属�?
    }
  });
});</code></pre>
  </div>
 ...
 class="interactive-demo">
  <div class="demo-header">
    ...
    <div class="demo-controls">
      ...
      <button class="demo-btn no-overwrite-btn">无覆�?/button>
...
gsap.killTweensOf(".box"); // 终止元素上的所有动�?
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td>完全替换现有动画</td>
        <td>overwrite: "all"</td>
        <td>终止元素上的所有动画，全新开�?/td>
      </tr>
      <tr>
        <td>相同类型的重复动�?/td>
        <td>使用ID标记</td>
        <td>相同ID的动画会自动覆盖，便于管�?/td>
      </tr>
      <tr>
        <td>复杂多元素动�?/td>
        <td>动画管理系统</td>
...
<div class="best-practices">
  ...
  <div class="practice-item">
    ...
    <div class="practice-content">
      ...
      <p>不要依赖默认行为，明确设�?code>overwrite</code>参数，让动画行为更可预测，特别是在复杂应用中�?/p>
    </div>
  </div>
  ...
  <div class="practice-item">
    ...
    <div class="practice-content">
      <h4>使用ID组织动画</h4>
      <p>为相关动画分配相同的ID，使�?code>id</code>参数标记，这样可以更轻松地组织和控制动画组�?/p>
    </div>
  </div>
  ...
  <div class="practice-item">
    ...
    <div class="practice-content">
      <h4>复用动画实例</h4>
      <p>存储动画引用并使�?code>play()</code>�?code>pause()</code>�?code>reverse()</code>等控制方法，而不是创建新动画�?/p>
    </div>
  </div>
...
- **overwrite参数**：控制动画如何覆盖现有动�?
- **动画ID**：组织和管理相关动画
- **killTweensOf方法**：精确控制动画终�?
- **动画控制方法**：如play()、pause()、reverse()�?
    ...
    noOverwriteBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        gsap.to(demoBox, {
          ...
          overwrite: false,
          onStart: () => {
            demoInfo.textContent = "无覆�? 两个动画同时运行，都会影响元�?;
          }
        });
      }, 1000);
    });
    ...
    autoOverwriteBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        gsap.to(demoBox, {
          ...
          overwrite: "auto",
          onStart: () => {
            demoInfo.textContent = "智能覆盖: 只有backgroundColor被覆盖，x、y和rotation不冲�?;
          }
        });
      }, 1000);
    });
